...
void GazeboImuPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    ...
    getSdfParam<std::string>(_sdf, "imuTopic", imu_topic_, mav_msgs::default_topics::IMU);
    ...
    getSdfParam<double>(
        _sdf, "gyroscopeBiasRandomWalk", imu_parameters_.gyroscope_random_walk, imu_parameters_.gyroscope_random_walk);
    ...
    this->resetEvent_ = event::Events::ConnectTimeReset(boost::bind(&GazeboImuPlugin::OnTimeReset, this));
    //==============================================//
    ...
    for (int i = 0; i < 9; i++) {
        ...
        case 0:
            imu_message_.add_angular_velocity_covariance(
                imu_parameters_.gyroscope_noise_density * imu_parameters_.gyroscope_noise_density);
...
void GazeboImuPlugin::AddNoise(Eigen::Vector3d* linear_acceleration, Eigen::Vector3d* angular_velocity, const double dt)
{
    ...
    for (int i = 0; i < 3; ++i) {
        gyroscope_bias_[i]
            = phi_g_d * gyroscope_bias_[i] + sigma_b_g_d * standard_normal_distribution_(random_generator_);
