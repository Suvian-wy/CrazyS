...
void GazeboMotorModel::CreatePubsAndSubs()
{
    ...
    command_sub_ = node_handle_->Subscribe(
        "~/" + namespace_ + "/" + command_sub_topic_, &GazeboMotorModel::ControlVelocityCallback, this);
    ...
    wind_speed_sub_ = node_handle_->Subscribe(
        "~/" + namespace_ + "/" + wind_speed_sub_topic_, &GazeboMotorModel::WindSpeedCallback, this);
...
void GazeboMotorModel::ControlVelocityCallback(GzCommandMotorSpeedMsgPtr& command_motor_speed_msg)
{
    ...
    ref_motor_rot_vel_ = std::min(static_cast<double>(command_motor_speed_msg->motor_speed(motor_number_)),
